...
<!-- AUTO-LOG:START -->
<!-- Regenerated by running `node scripts/update-loop-log.js` — do not hand-edit this block, it will be overwritten next run. Last updated: 2026-09-09T19:48:25.310Z -->

| Commit | PR | Message | Date |
|---|---|---|---|
| [`03a7968`](https://github.com/nishant7k/LoopEngineeringClaude/commit/03a796840afce2a22650a81d26b7d719163938da) | [#16](https://github.com/nishant7k/LoopEngineeringClaude/pull/16) | Fix loop-live.html: clock-dependent PR detection, and a false-sequential layout | 2026-09-09 |
| [`bf4826b`](https://github.com/nishant7k/LoopEngineeringClaude/commit/bf4826b675baa939e3e12873e6fd8370929c4d49) | [#15](https://github.com/nishant7k/LoopEngineeringClaude/pull/15) | Move the app's HTML/CSS/JS/config under src/ for a cleaner repo root | 2026-09-09 |
| [`3399aee`](https://github.com/nishant7k/LoopEngineeringClaude/commit/3399aeeb3c09c90615b20f81b16427438b5123f3) | [#14](https://github.com/nishant7k/LoopEngineeringClaude/pull/14) | Remove the auto-commit CI job — branch protection blocks it by design | 2026-09-09 |
...
- PR #13 (`5cf169e`) — added `scripts/update-loop-log.js`; its CI auto-commit design turned out to be broken (see PR #14 below).
- PR #14 (`3399aee`) — removed the auto-commit CI job: branch protection blocks any push to `main` without status checks, including a bot's, so it failed on its first real run. `scripts/update-loop-log.js` is now a manual pre-demo command instead.
- PR #15 (`bf4826b`) — moved `index.html`, `loop-live.html`, `monitoring.html`, `before-after.html`, `feature-flags.json`, `css/`, `js/` under `src/` for a cleaner repo root. `demo-baseline` retagged afterward to match.
...
## Iteration 10 — loop-live.html was lying about the pipeline shape, and could silently miss a real PR

Reported live, from an actual screenshot: the stepper was stuck on
"Code · Commit · Push" despite a real PR having been opened, and the
Lint → Test → Security layout implied a strict sequential chain that
doesn't match the YAML.

- **PR #16** — two real, distinct bugs:
  1. PR detection compared `Date.now()` (the browser's clock, captured
     when "Start watching" is clicked) against a PR's `created_at` (the
     server's clock). Client clock drift, or a backgrounded browser tab
     whose `setTimeout` polling gets throttled before it can check, can
     make a genuinely new PR look older than baseline and never get
     detected. Fixed by comparing PR **numbers** instead — no clock
     involved at all — plus a `visibilitychange` listener that forces an
     immediate poll when the tab regains focus.
  2. The stepper drew Lint → Test → Security as one sequential line.
     Lint/Test are independent jobs in `ci-cd.yml` with no dependency
     between them, and Security is a separate workflow
     (`security-review.yml`) that starts in parallel with `ci-cd.yml`,
     not after it. Now rendered as one "PR Checks (run in parallel)" box
     with 3 simultaneous sub-rows, matching the real job graph.
  - Verified locally first with a real Playwright script (mocked
    `api.github.com` via route interception to control timing, drove the
    exact "PR appears after watching starts" scenario, confirmed
    detection with zero timestamp comparisons and zero console errors).
  - Then verified against the **live deployed page**, no mocks: opened
    `https://nishant7k.github.io/LoopEngineeringClaude/loop-live.html`
    with a real Playwright session, clicked **Start watching** for real
    (baseline captured before this PR existed), then opened **PR #17**
    (this doc update) from a separate process. The live page's "Pull
    Request" step flipped to done in **~29s** — well inside the
    unauthenticated poll cadence — showing the real PR #17 title and
    link, with zero console/page errors. No timestamp comparison was
    involved anywhere in that path; detection ran purely on the PR
    number not being in the baseline snapshot.
